...
public class CWE78TestCase {

	private static final Logger LOGGER = LoggerFactory.getLogger(CWE78TestCase.class);
	...
	public static void main(String[] args) {
		Path path = Paths.get("/Users/leolu/Downloads/cwe-top25-5/match_25_cwe_78");

		DefaultContext build = IContext.builder()
			.withTaskName("sast-java")
			...
			.withTargetAllFilePath(recordAllFiles(path))
			.build();
		...
		CWE78TestCase cwe476TestCase = new CWE78TestCase();
		cwe476TestCase.test(pointerAnalysis);
		cwe476TestCase.runOnModule(propagationTransform.getCgNodes());
	}

	public static Set<Path> recordAllFiles(Path rootPath) {
		Set<Path> files = new HashSet<>();
		try {
			Files.walkFileTree(rootPath, EnumSet.noneOf(FileVisitOption.class), Integer.MAX_VALUE, new SimpleFileVisitor<>() {
				@Override
				...
				public FileVisitResult visitFileFailed(Path file, IOException exc) {
					LOGGER.error("Failed to visit file: [{}], reason: [{}]", file, exc.getMessage());
					return FileVisitResult.CONTINUE;
				}
			});
		} catch (IOException e) {
			LOGGER.error("Error occurred while walking the file tree from root path [{}]: {}", rootPath, e.getMessage(), e);
		}
		return files;
	}

	public void test(PointerAnalysis<InstanceKey> pa) {
		this.sourceSinkFile = "/opt/config/sources_and_sinks/cwe_78_os_cmd_inject.json";
	...
	protected ITaintSolver createTaintSolver(CallGraph callgraph) throws IOException {
		...
		if (isDir(wrapperFile)) {
			wrapper = new SummaryTaintWrapper("CWE_78", wrapperFile, callgraph, pa);
