...
import com.repograph.taint.support.framework.spring.RequestMethod;
...
public class RequestMapping implements IAnnotation {
	private String name;
	...
	public RequestMapping(ElementValue name, ElementValue value, ElementValue path, ElementValue method,
						  ElementValue params, ElementValue headers, ElementValue consumes, ElementValue produces) {
		if (value != null) {
		...
		if (method != null) {
			assert path instanceof ArrayElementValue;
			ElementValue[] vals = ((ArrayElementValue) method).vals;
			this.method = new RequestMethod[vals.length];
			for (int i = 0; i < vals.length; i++) {
				ElementValue subVal = vals[i];
				assert subVal instanceof EnumElementValue;
				switch (((EnumElementValue) subVal).enumVal) {
					case "GET":
						this.method[i] = RequestMethod.GET;
						break;
					case "HEAD":
						this.method[i] = RequestMethod.HEAD;
						break;
					case "POST":
						this.method[i] = RequestMethod.POST;
						break;
					case "PUT":
						this.method[i] = RequestMethod.PUT;
						break;
					case "PATCH":
						this.method[i] = RequestMethod.PATCH;
						break;
					case "DELETE":
						this.method[i] = RequestMethod.DELETE;
						break;
					case "OPTIONS":
						this.method[i] = RequestMethod.OPTIONS;
						break;
					case "TRACE":
						this.method[i] = RequestMethod.TRACE;
						break;
					default:
						System.err.println("illegal RequestMethod in " + method);
						break;
				}
			}
		}
