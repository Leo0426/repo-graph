...
**能力边界**：面向静态启发式分析，不做完整编译器语义分析——粗中粒度对检索与审计够用；需要最高精度的场景由 WALA IFDS 精确污点引擎补位。
...
- **漏洞管理三路互补** — 静态规则（快）+ 跨过程污点（精，源码级 + WALA IFDS 字节码级）+ 离线 CVE，发现记录走状态机确认，完全离线。
...
| 漏洞 | 代码扫描（9 条 CWE 规则）、污点扫描、精确污点扫描（WALA IFDS）、依赖 CVE（离线 Advisory） |
...
repograph-taint-engine/ WALA IFDS 精确污点引擎（独立进程，JDK 21；详见模块内 README）
```
...
| **JDK 25** | 需开启 `--enable-native-access=ALL-UNNAMED`（Tree-sitter FFM）；精确污点扫描另需带 jmods 的 JDK（如 JDK 21） |
...
- `./gradlew :repograph-taint-engine:installDist` — 精确污点引擎（供精确扫描子进程调用）
...
| `POST` | `/api/v1/vulns/scan/{code,taint,taint/precise,deps}` | 四路漏洞扫描 |
...
**精确污点扫描**默认关闭：需先 `./gradlew :repograph-taint-engine:installDist`，再在 `repograph.taint.precise` 配置带 jmods 的 JDK 路径与引擎 lib 目录并置 `enabled: true`。引擎以独立子进程运行，与 app 的 JDK 25 互不干扰。REST 接口见[用户手册 §5](docs/user-manual.md)。
...
**第二阶段已完成**：`search_graphrag` / `search_keyword` / `build_context_pack` / `triage_finding` MCP 工具、研判反馈闭环、安全与 Agent 定向工具、精确污点引擎接入。
...
- 精确污点扫描要求目标可编译（提供 classes/jar），且引擎须运行在带 jmods 的 JDK（JDK 17/21 可用，JDK 25 不含 jmods）
